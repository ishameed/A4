...
file_name = '../hp_results/fl_hp_30_gamma_pi.pickle'
grid_size = 30
hole_states = []
...
def run_frozen_lake(P, R, gamma, eval_type):
    print("EVAL TYPE: ", eval_type)
    # Create and run the MDP model using Value Iteration
    initial_policy = np.random.randint(4, size=(grid_size*grid_size))
    vi = hiive.mdptoolbox.mdp.PolicyIteration(P, R, gamma = gamma, eval_type = eval_type, policy0 = initial_policy)
    vi.run()
...
def objective(trial):
    ...
    eval_type = trial.suggest_int("eval_type", low = 0, high = 1)
    gamma = trial.suggest_discrete_uniform('gamma', 0.8, 0.9999, 0.001)
    
    # Generate the forest MDP using mdptoolbox example
    ...
    avg_value = run_frozen_lake(P, R, gamma, eval_type)
...
for run_num in range(num_optimization_runs):
    ...
    eval_types = [trial.params['eval_type'] for trial in study.trials]
    eval_type_runs[run_num] = eval_types
